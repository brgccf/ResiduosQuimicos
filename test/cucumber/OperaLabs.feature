Feature: Operar com laboratorios disponíveis no sistema.
  ...
  Scenario: Solicitação de acesso a laboratórios por usuário do tipo Facilitador
    ...
    And os laboratórios "LEA" e "LAMAI" estão disponíveis para associação
  ...
  Scenario: Aprovação de acesso a laboratório para um usuário do tipo Facilitador
    ...
    And Existe uma solicitação de acesso ao laboratório "LEA" feita pelo usuário do tipo Facilitador "Pedro"
  ...
  Scenario: Tentativa de solicitação de acesso a laboratório com usuário associado
    ...
    And o laboratório "LEMAI" está disponível para associação
    When eu tento solicitar associação de "Pedro" ao laboratório "LEMAI"
  ...
  Scenario: Confirmação de solicitação de acesso a laboratório para usuário Facilitador
    ...
    And o laboratório "LEMAI" está disponível para associação
    When eu tento solicito associação de "Marcos" ao laboratório "LEMAI"
    Then eu posso ver uma mensagem de confirmação indicando a solicitação de "Marcos" para acessar o laboratório "LEMAI"